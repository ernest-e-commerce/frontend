...
import { hasUserRated, toggleProductRating } from "../api/productService";
...
const ProductCard = ({ product }) => {
  ...
  useEffect(() => {
    ...
    hasUserRated(product._id, user.id)
      .then((rated) => {
        if (!cancelled) setIsRated(rated);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const handleRate = async (e) => {
    ...
    const optimisticRated = !prevRated;
    const optimisticCount = optimisticRated ? prevCount + 1 : prevCount - 1;
    setIsRated(optimisticRated);
    setRatingCount(optimisticCount);

    try {
      await toggleProductRating(product._id, user.id);
      toast.success(optimisticRated ? "Thanks for your rating!" : "Rating removed");
    } catch (err) {
      ...
      <div className="relative bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-200/80">
        <div className="relative pt-[100%] bg-gray-100 rounded-t-lg overflow-hidden">
          ...
          >
            <StarIcon
              filled={isRated}
              className={`w-5 h-5 ${isRated ? "text-yellow-500" : "text-gray-300"}`}
            />
